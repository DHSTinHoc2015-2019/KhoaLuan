				...
				<div class="col-xs-12 col-sm-12 col-md-12 col-lg-6 col-xl-12">                      
                    <div class="card mb-3">
                        <div class="card-header">
                            <h3><i class="fa fa-plus"></i> Chỉnh sửa video</h3>
                        </div>
                            
                        <div class="card-body">
                        	<form v-on:submit.prevent="editLibraryVideo">
                            <div class="row">
								<div class="col-md-6">
	...
	export default {
		...
		created(){
			...
			this.axios.get(`/api/library/show/${this.$route.params.id}`).then((response) =>{
				...
				this.library_video = response.data
			}).catch((error) => {
				console.log(error)
			})
		},
		methods: {
			editLibraryVideo(){				
				...
				axios.defaults.headers.common['Authorization'] = 'Bearer ' + localStorage.getItem('tpack.jwt')
				let uri = `/api/libraryvideo/edit/` + this.$route.params.id
