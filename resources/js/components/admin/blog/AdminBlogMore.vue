							...
							<div class="card-footer">
								<p style="color: #999;">
									<span>
										...
										<a href="#" class="mr-2">{{ blogs.name }}</a>
									</span>
									<span>
    ...
    <ul id="comment-list" class="main">
        <li class="comment by-current-user">
            ...
            <ul class="child-comments" v-for="(value_detail, index_detail) in commentdetail">
				...
				<!-- edit commentdetails -->
                <li class="comment by-current-user edit" v-if="value_detail.id_comment_blog == value_comment.id && value_detail.id_user == users.id && editDetail[index_detail]">
                    <div class="comment-wrapper">
...
<script>
	...
	export default {
		...
		created(){
			...
			this.axios.get(`/api/blog/showwithcomment/${this.$route.params.id}`).then((response) => {
		...
		methods:{
			...
			getData(){
				axios.defaults.headers.common['Content-Type'] = 'application/json'
				axios.defaults.headers.common['Authorization'] = 'Bearer ' + localStorage.getItem('tpack.jwt')
				axios.get(`/api/blog/showwithcomment/${this.$route.params.id}`).then((response) => {
					this.blogs = response.data.blog
					this.comment = response.data.comment
					this.commentdetail = response.data.commentdetail
					this.countComment = response.data.countComment
