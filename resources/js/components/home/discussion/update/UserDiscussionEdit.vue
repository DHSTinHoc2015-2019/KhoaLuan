				...
				this.axios.post(`/api/discussion/update/${this.$route.params.id}`, this.discussion).then((response) => {
					if(response.data.status){
						alertify.set('notifier','position', 'buttom-right');
		 				alertify.success('Cập nhật thảo luận thành công');
						this.$router.push({ path: '/thaoluan/' + this.discussion.id_discussion_type + '/' + this.$route.params.id})
					} else {
						alertify.set('notifier','position', 'buttom-right');
		 				alertify.error('Cập nhật thảo luận thất bại');
					}
				}).catch((error) => {
					console.log(error)
				})
